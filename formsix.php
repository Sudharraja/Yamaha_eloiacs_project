...
$deleted = 0;  
  ...
  $sql = "INSERT INTO `project` (INVOICEDATE, CHASSISNO, VEHICLEMODEL, CUSTOMERNAME, PAYMENTTYPE, BRANCHESDEALERS, CUSTOMERCODE, PAYMENTMODE, Del_status)
  VALUES ('$invoicedate', '$chassisno', '$vehiclemodel', '$customername', '$paymenttype', '$branchesdealers', '$customercode', '$paymentmode', '$deleted')";  
...
if (isset($_REQUEST['import-excel'])) {
    ...
    if ($extension == 'xlsx' || $extension == 'xls' || $extension == 'csv') {
        ...
        foreach ($data as $row) {
            $name = date('Y-m-d', strtotime($row['0']));
            $dob = $row['1'];
            $email = $row['2'];
            $mobile = $row['3'];
            $pay = $row['4'];
            ...
            $insert_query = mysqli_query($conn, "INSERT INTO project (INVOICEDATE, CHASSISNO, VEHICLEMODEL, CUSTOMERNAME, PAYMENTTYPE, BRANCHESDEALERS, Del_status) VALUES ('$name', '$dob', '$email', '$mobile', '$pay', '$branch','$deleted')");
